...
export function NavbarSearch() {
  const { rollBackSearch, seeDBGamesOnClick, handleSubmit, inputRef } =
    useSearch();
  return (
        ...
        <div className="flex w-full md:w-2/4">
          <form action="" className="w-full" onSubmit={(e) => handleSubmit(e)}>
            <input
              ...
              id="name"
              type="search"
              className="border-4 w-2/4  p-2 border-black focus:outline-none"
              placeholder="Search..."
            />
            <button className="border-4 w-2/4 px-1 p-2 border-black border-l-0 bg-red-700 text-white hover:bg-red-800 hover:border-black ease-in-out">
              Search
            </button>
          </form>
        </div>
